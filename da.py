...
class Stocker():
    ...
    def plot_stock(self, start_date=None, end_date=None):
        ...
        plt.title('%s Stock Price History' % self.symbol);
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares=1000):
        ...
        print(
            'When the model predicted an increase, the price increased {:.2f}% of the time.'.format(increase_accuracy))
        ...
        print(
            'The Buy and Hold strategy profit =         ${:.2f}.'.format(float(test.ix[len(test) - 1, 'hold_profit'])))
    ...
    def changepoint_date_analysis(self, term=None):
        ...
        plt.title('Stock Price and Search Frequency for %s' % term)
        plt.show()
